...
struct EntryPresetOperationsTests {
    ...
    func starterPresetsPreserveFamiliarExamples() {
        ...
        #expect(presets.compactMap(\.starterIdentity) == EntryStarterPreset.allCases)
        #expect(presets.map(\.id.uuidString) == [
    ...
    @Test
    func customPresetUsesItsStoredTitleForDisplay() {
        let preset = EntryPreset(
            ...
            origin: .custom,
            id: EntryStarterPreset.bag.id
        )

        #expect(preset.starterIdentity == nil)
        #expect(preset.displayTitle == "Travel bag")
    }
