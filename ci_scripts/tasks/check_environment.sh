...
usage() {
  cat <<'EOF' >&2
Usage: bash ci_scripts/tasks/check_environment.sh --profile <swiftlint|rules|test|build>
EOF
}
...
check_swiftlint_environment() {
  local project_file="Fluel.xcodeproj/project.pbxproj"
  ...
  if ! grep -q --fixed-strings "https://github.com/SimplyDanny/SwiftLintPlugins" "$project_file"; then
    record_failure "Fluel.xcodeproj is missing the SwiftLintPlugins package dependency."
    record_next_step "Add https://github.com/SimplyDanny/SwiftLintPlugins to Fluel.xcodeproj."
  fi
}

check_test_environment() {
  ensure_command "swift" "Install Xcode or the Swift toolchain so package tests can run."

  if [[ ! -f "FluelLibrary/Package.swift" ]]; then
    record_failure "Missing file: FluelLibrary/Package.swift"
    record_next_step "Restore FluelLibrary before running library tests."
  fi
}
...
check_build_environment() {
  ...
  if [[ ! -f "Fluel.xcodeproj/project.pbxproj" ]]; then
    record_failure "Missing file: Fluel.xcodeproj/project.pbxproj"
    record_next_step "Restore Fluel.xcodeproj before building the app."
  fi
}
