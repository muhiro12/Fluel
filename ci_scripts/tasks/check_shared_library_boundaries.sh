...
forbidden_import_matches=$(
  rg --line-number \
    '^import (SwiftUI|TipKit|PhotosUI|WidgetKit|MHPlatform|MHAppRuntime|MHAppRuntimeDefaults|MHAppRuntimeAds|MHAppRuntimeLicenses|MHMutationFlow|MHReviewPolicy|MHUI|AppIntents|StoreKit|StoreKitWrapper|GoogleMobileAds|GoogleMobileAdsWrapper|UserNotifications)$' \
...
if [[ -n "$forbidden_import_matches" ]]; then
  ...
  echo "If shared code ever needs MHPlatform access, stop at MHPlatformCore or granular core-safe modules." >&2
